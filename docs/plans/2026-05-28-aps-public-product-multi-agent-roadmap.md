...
AI 應先顯示所有本機可見來源的總覽，再列細節；不完整或不一致時，先要求補交或共識確認，不預設 consume。背景自動通知、桌面通知、平台排程不屬近期版本。
...
同步資料夾本身不能保證「即時彈窗」或「自動喚醒 AI」。本文件把通知拆成兩層:摘要式人類通知是 Reliable Peer Handoff 必做 UX;檔案式 `_notify`、`watch`、桌面通知、平台排程、背景檢查與 Telegram bot 自動代發屬延後或 opt-in 方向。近期核心只保留手動 `check Hub` 與 copy-ready 人類通知。Telegram、WhatsApp、Email 都只是把摘要式通知送到人手上,不是控制對方 AI 的渠道。通知內容必須包含重點摘要與注意事項,不能只列交接包 ID。以下方向只作中期設計備忘。
...
| Claude Code Channels | 官方 Channels 文件列出 Telegram plugin 可把外部訊息推入已運行的 Claude Code session,但屬 opt-in / research preview,需要 session 開着、用戶配置與 allowlist。 | 只可作熟手進階路線參考;Reliable Peer Handoff 主路徑仍應是人收到通知後,自行在本機 AI 輸入 `check Hub`。 |
...
|---|---|---|---|
| A2A protocol | 使用 Agent Card 做能力與身份發現；任務、訊息、artifact、狀態更新分開；push notification 屬可選能力，未宣告支援時必須回錯。 | `_peers/agents/<agent_id>.json` 應接近本地 Agent Card；packet / outbox / ack / notify 要分工清楚；`watch` 必須是明確能力，不可假裝所有環境都支援。 | 不把 HTTP、JSON-RPC、webhook、OAuth 或 server endpoint 變成 APS 核心要求。 |
...
| 把近期目標誤寫成多人平台 | 近期主線只可寫成 Reliable Peer Handoff：project-level peers + 單收件 packet + sent/status。真正多人、notify、watch、平台排程均屬延後方向。 |
...
16. `watch` 是否能保證只讀？
17. watch 是否可先不依賴 notify，只掃 packet / outbox / ack？
18. notify、outbox、ack 三者不一致時，誰是判斷真源？
19. Dropbox / OneDrive 的本機同步行為是否已用實機驗證？
20. Claude Code / Codex / Antigravity 的排程適配是否逐平台記錄權限、限制與取消方法？
21. 公開頁是否仍清楚標示真正多人、notify 與 watch 是延後路線，而不是 0.2.x 已有功能？
...
4. 任何公開文檔只可說 0.2.8 已支援目前二人基線；Project Peers 是 v0.2.9 候選方向；Project Context Index 是 Project Peers 之後的背景索引方向；真正多人、notify、watch、自動檢查是延後路線，不可寫成 0.2.x 已支援。
