...
| 自動通知延後 | 近期只保留 copy-ready 人類通知與手動 `check Hub`；檔案式 notify、桌面通知、定時喚醒 AI、背景檢查全部延後。 |
| 人類可保留打斷控制權 | APS 可以生成可直接複製的通知，也可以提供定期檢查，但不預設代替人類發 WhatsApp、Email 或跨平台訊息。 |
...
1. 讀取本地 APS 設定與可用聯絡名單。
2. 確認 `Jay` 對應哪一個 confirmed agent id。
3. 執行健康檢查與收件狀態檢查。
4. 整理共同目標、本方任務、對方任務、交叉點、請對方做的事、證據與風險。
5. 做發送前完整性預檢。
6. 向使用者顯示摘要、對象、topic、寫入位置與預檢結果。
7. 取得明確確認後才寫入 Hub。
8. 寫入後產生 copy-ready 通知文字，讓使用者透過既有渠道通知對方 `check Hub`。

若使用者輸入的是群組名稱，例如 `launch_team`，AI 不應直接建立 `from_launch_team`。正確做法是先展示群組成員與交接對象，再由使用者確認是發給全部成員、某幾位成員，還是發給一個真正存在的 team agent。第一版 Contacts 只把群組展開成多個單收件 packet，不做同一 packet 多個收件人。
...
## 五、檔案式通知與 watch 的延後方向

同步資料夾本身不能保證「即時彈窗」或「自動喚醒 AI」。近期版本不應做檔案式通知或自動化 watch；只保留手動 `check Hub` 與 copy-ready 人類通知。以下方向只作中期設計備忘。
