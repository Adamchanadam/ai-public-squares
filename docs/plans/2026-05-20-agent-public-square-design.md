...
3. **本地橋接(各自,薄薄一層)** —— 每個 agent 嗰邊一個規則包(`bridge-pack`),將自己 SSOT 嘅產出打包送入交換區,以及接收返來嘅嘢登記入自己 task。橋接只做翻譯同搬運。**呢層由 agent 程式運行;用戶嘅操作對住 agent,唔對住共享資料夾**(actor 區分見 §11)。
...
協定全文用以下三個識別碼,定義一次,後面唔重複:

- `<agent_id>` — 一個穩定嘅小寫 snake_case 字串,代表一個 agent(例:`adam`、`jay`)。同一個字串既係資料夾名(`from_<agent_id>/`、`_ack/<agent_id>.ack.json`),亦係 packet YAML 入面 `from:` / `to:` / `owner:` 嘅值。
- `<project_slug>` — 穩定嘅小寫 snake_case 字串,代表一個 project。同一字串既係資料夾名,亦係 packet `project:` 欄。
- `<packet_id>` — `<UTC-yyyymmddThhmmssZ>__<short_snake_topic>` 形式嘅字串。**唔含任何版本後綴。**

**`<version>` 係另一個獨立整數**,起始為 1,每次 `revise` +1。

**協定全文嘅 canonical 聯合鍵係 `(packet_id, version)` 一對。** 同一對 key 喺四個地方有四種序列化形態:

| 出現位置 | 形態 |
|---|---|
| Packet 資料夾名 | `<packet_id>__v<N>/` |
| `packet.md` YAML | 兩個獨立欄:`packet_id: <packet_id>` 同 `version: <N>` |
| `outbox.log.md` 行 | `<packet_id> v<N>`(空格,唔係 `__v`) |
| `ack.json` entry | 兩個獨立欄:`packet_id` 同 `version` |

`supersedes:` 欄用資料夾名形態:`<packet_id>__v<N>`(或 `null`)。任何 implementation 讀任何一種形態都必先 canonicalize 成 `(packet_id, version)` 才作比較。
...
**`items[].status` 係 publish 一刻嘅 snapshot,唔係 live 狀態。** 收方做完一個 item 之後唔可以反過去改原 packet 嘅 status;要更新狀態 = 出新版本(`revise`)。`supersedes:` 寫舊版本嘅資料夾名形式(`<packet_id>__v<N>`),保持 `(packet_id, version)` 聯合鍵清晰。
...
- `revise` — 發佈新版本 `(packet_id, vN+1)`,新 packet 嘅 `supersedes:` 必明寫舊版本。
- `close` — 將「整個 logical `packet_id`」(所有版本)標記為已結。帶 `reason:`。`close` 之後,即使收方未 ack,收方都唔應再處理該 packet。
- `withdraw` — 撤回一個已發佈嘅版本(收方仍未 ack 之前)。帶 `reason:`。Packet 資料夾保留喺磁碟(不可變),但收方嘅 pending 計算必須過濾佢出。發方有責任先睇 `_ack/<receiver>.ack.json`;若收方已 ack 咗 `(packet_id, version)`,`withdraw` 係無效嘅,要改用 `close` + 一個有修正內容嘅 `revise`。
...
2. 按 `packet_id` 分組。若同組任何一行係 `close` → 整組丟棄(任何版本都唔再列)。
3. 餘下每組,只睇 `publish` / `revise` 事件。
4. 每組取最大 `version` 為 winner(舊 `publish` 被新 `revise` 蔽掉)。
5. 若 `(packet_id, winner_version)` 喺同組亦出現過 `withdraw`,過濾走;否則對住自己 `_ack/<me>.ack.json` `consumed` 比對,匹配同一對 `(packet_id, version)` → 已消化,差集為空 → 唔列;比對唔到 → 待處理。

**「消化(consume)」嘅明確定義:** 收方喺自己 `_ack/<receiver>.ack.json` `consumed[]` 加咗一個 entry,內含 `(packet_id, version)` + `at` + 一句 `result:` 描述做咗咩。冇 `result:` 嘅 ack 唔算消化。
...
**Mid-session 手動重跑(由用戶觸發):** 收方 agent 喺 session 中間,如果用戶講固定觸發詞(`check Hub` / `未消化` / `睇下 Hub 有冇新嘢`),agent 即刻 re-run 同樣計算式。呢個係手動 trigger,唔係自動 poll —— agent 程式唔會主動監察共享資料夾,設計上保留人手叫一聲嘅噪音控制(§8)。要做自動 push 通知,要本地 daemon,出咗純檔案範圍。
...
本協定全文嚴格區分四個 actor。本協定文字入面提及「Adam 出包」「Jay 撿包」之類嘅 phrasing,實質係指對應嘅 agent 程式做 file IO;用戶本人嘅動作會明寫「用戶 X 發 WhatsApp」「用戶 X 決定 / 揀」之類。

| Actor | 身份 | 負責邊類動作 |
|---|---|---|
| 用戶 Adam | 真人 | 寫內容、揀做咩、揀幾時做、發 WhatsApp、決定是否撤回 / 出新版本 |
| Agent Adam | Adam 部機嘅 agent 程式(Claude Code / Codex CLI 之類) | 對共享資料夾做 file IO(寫包、append ledger、update ack);跑啟動讀序;呈現待辦畀 Adam |
| 用戶 Jay | 真人 | 同上,Jay 嗰邊 |
| Agent Jay | Jay 部機嘅 agent 程式 | 同上,Jay 嗰邊 |

**兩條 channel,各做各嘢:**
- Hub 上嘅 file IO channel:`Agent Adam ↔ 共享資料夾 ↔ Agent Jay`。兩個 agent 程式各自單寫自己 lane 嘅檔,過 Drive 同步至對方部機。
- WhatsApp channel:`用戶 Adam ↔ 用戶 Jay`。一句固定模板做 trigger,等收嘅人開新對話 / 喺對話入面叫 agent 去 Hub 撿。

呢兩條 channel 平行,各做各嘢。WhatsApp 唔由 agent 程式發 — 由用戶本人發。Hub 上嘅檔唔由用戶本人寫 — 由 agent 程式寫。
...
6. ack.json schema 同「消化」定義(本文 §6.3)。
7. Receiver pending 計算式(排除 `close` 組、過濾 `withdraw` 版本)(本文 §6.3)。
8. 啟動 / 收尾橋接職責(本文 §7)。
9. 固定 WhatsApp 模板(本文 §8)。
10. 敏感資料禁區(本文 §14)。
11. 修改本協定嘅程序(任何協定改動必由雙方 agent owner 共同議決,寫入 `_hub/CHANGELOG.md`)。
...
- 2026-05-21:輕量補丁 — §11 加 Actor 區分定義同兩條 channel 分工;§4 加 cross-ref 至 §11;§7 加 mid-session trigger caveat(手動 trigger,唔係自動 poll)。配合用戶面向 walkthrough 嘅 actor 區分重整,協定設計本身不變。
