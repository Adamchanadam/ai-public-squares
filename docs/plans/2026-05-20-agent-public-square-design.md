...
### 階段 1:Hub Root 結構建立(可由 User A 一邊獨力完成)
...
- 建 `<hub_root>/<project_slug>/` 之初始 lane 骨架(`from_<User_A>/`、`from_<User_B>/`、`_ack/`)。
...
- Demo agent A 出一個假 packet → 同步去 Drive → Demo agent B 啟動 → 確認自動列待辦 → Demo agent B 出回包 → Demo agent A 啟動 → 確認自動列「B v1 已回」。
- 驗收條件:無人手搬檔、無人手解釋上下文、無 Drive 衝突副本。

### 階段 4:正式接入真實 runtime(將來)

- User A 真實 runtime 加 `aps-bridge.md`(僅 demo 跑通後);實際工作目錄之路徑由 User A 自行決定。
- User B 真實 runtime 同等動作;實際工作目錄之路徑由 User B 自行決定。
