...
| npm package change (`package.json`, `bin/aps.js`, version bump, `npm publish`) | `package.json` version / bin path / dependencies / files 含 `bin/` `skills/` `examples/`;`bin/aps.js` 各 sub-command handler(`init` / `bridge-pack` / `--help` / unknown);README install command + Build status table 反映 latest version;GitHub release / tag 對應 npm version | smoke test:跑 `node bin/aps.js [sub-command]` 各路徑(`--help` / `init` / `bridge-pack` / `bridge-pack --role B` / `bogus`)— 前 4 路徑 exit 0,`bogus` exit 1;`bridge-pack` output 為 valid markdown(含 `# APS Bridge Pack` heading 約 120 行);`package.json` valid JSON + `npm pack --dry-run` 出現預期 file list 含 `examples/demo-agent-{a,b}/dev/rules/aps-bridge.md`;README install command 同 `package.json` name 一致 |
