# APS 全面檢報告 — 0.2.3 guided setup candidate

日期: 2026-05-27
範圍: `@adamchanadam/aps` 0.2.3 pre-release 候選版,發布前本機全面檢
...
0.2.3 候選版已把新用戶主路徑改為互動式 `npx aps init`:工具逐步詢問角色、項目代號、雙方 agent id 與真實 Hub 路徑,列出寫入計劃,只在使用者輸入 `yes` 後寫入。全面檢期間發現一個日常協作 UX 缺口: `inbox` 原本沒有列出可直接複製的 `consume` 命令。已即時修正為每個 pending packet 顯示完整 `npx aps consume --packet-id ... --version ... --result ...` 命令,並以同機回歸重新驗證。

本報告不等同發布批准。`npm latest` 仍為 0.2.2;0.2.3 仍需 commit / push / tag / GitHub pre-release / npm publish 才可供 Adam 與 Jay 從 npm latest 試用。

## 三條主線

| 主線 | 狀態 | 證據 |
|---|---|---|
| 公開承諾一致性 | 通過 | README、本機 `docs/index.html`、教學中心、完整落地教學、QC 卡片、CLI help、skill spec 均指向 0.2.3 候選版互動式設置,並保留自然語言日常操作與補救流程仍屬前期測試的邊界。候選版 / 已發布掃描沒有把 0.2.3 寫成 npm latest。 |
...
| 流程 | 狀態 | 證據 |
|---|---|---|
| 零認知讀者入口流程 | 通過 | README → `docs/index.html` → `docs/guides/index.html` → walkthrough 均說明目前可試的是 0.2.3 候選版互動式 CLI,並提醒 npm latest 尚未是 0.2.3。 |
...
| 3. 本次 acceptance grep | 通過 | HTML `.md` hyperlink audit 0 hits;候選版 / 已發布 wording scan 0 hits;治理詞彙掃描確認公開 HTML 反映三條主線與四條流程。 |
...
| 6. 三處交叉閱讀 | 通過 | 公開入口、walkthrough、Bridge Pack / skill 對互動式 setup、Hub skeleton、packet、ack、close / withdraw 的敘述一致;0.2.3 仍標示為候選版。 |
...
- `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json`:latest 仍為 0.2.2,bin `aps`,fileCount 14。
...
1. 0.2.3 尚未 push / tag / GitHub pre-release / npm publish;Adam 與 Jay 不能用 npm latest 測 0.2.3。
2. GitHub Pages live readback 本輪受阻;發布後必須重試,不可預先宣稱線上頁已更新。

建議下一步:提交本次全面檢與 CLI UX 修正;之後如 Adam 明確批准,才執行 push、tag、GitHub pre-release、npm publish,並做 npm / Pages / GitHub release 讀回。
