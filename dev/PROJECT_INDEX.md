...
| Package manager | npm (`@adamchanadam/aps` 0.2.2 pre-release release-prep;published latest should verify as 0.2.2 after npm publish;`.aps/config.json` saved local config and short daily commands included; isolated skill rehearsal, throw-away zero-knowledge flow, one real Adam ↔ Jay Google Drive round-trip, and local 0.2.2 short-command regression passed) | 2026-05-27 |
...
| `docs/index.html` | non-developer project explainer; user-maintained entry page;current status reflects 0.2.2 pre-release CLI-first setup, short commands, and one completed maintainer-run Drive verification | communicating APS state to humans, e.g. status updates to a counterpart or other stakeholders | local path | 2026-05-27 |
...
| `docs/guides/aps-onboarding-walkthrough.html` | end-to-end User A + User B step-by-step onboarding teaching page;now uses the 0.2.2 CLI-first setup path with saved config / short-command flow and marks day-to-day natural-language flow as pre-release trial use | first-time user reading; pairing with Phase 4 plan during execution | local path | 2026-05-27 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + current 0.2.2 CLI-first path + deep-dive references | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-27 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-27 (`main` prepared for 0.2.2 push;tag `v0.2.2` pending until release command completes) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` expected to become 0.2.2 after publish) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-27 |
| GitHub release `v0.2.2` | public pre-release record for APS package version 0.2.2 | before telling users v0.2.2 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.2` | create / update only with explicit release intent | 2026-05-27 (pending creation in this release pass) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-27 (will need post-push readback for `0.2.2` wording) |
...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; disposable Hub `publish` → `inbox` → `revise` → `inbox` → `withdraw` → `inbox` → `publish` → `consume` → `close` → `doctor`; throw-away zero-knowledge tarball install flow; `node bin/aps.js bogus`; post-publish install probe; local 0.2.2 saved-config short-command flow and `config` command tests | every time `bin/aps.js` changes, before committing the change | 2026-05-27 (source path, packed-tarball path, isolated skill-level rehearsal, throw-away zero-knowledge flow, formal 🟡 外發前檢, 🔴 全面檢, real Drive round-trip, npm publish, install probe, local 0.2.1 `revise` / `withdraw` / `doctor` smoke pass, formal 0.2.1 外發前檢, and formal 0.2.2 外發前檢 passed) |
...
| Branch / commit | `main` / 0.2.2 release-prep commit pending;0.2.1 release commit `d9dd1aa1acf2b2f5c863056382543922c5c0aeb4`;tag `v0.2.1` points to release commit | 2026-05-27 |
...
| Uncommitted change summary | Expected clean after local 0.2.2 candidate commit;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
