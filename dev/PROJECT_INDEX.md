...
| `skills/` | APS skill source — `skills/aps/SKILL.md` 是 master source(Phase X-3 第二階梯:frontmatter + Section 1-3 起手 routing 已 land,Section 4-8 子流程展開待第三階梯);`npx @adamchanadam/aps init` 將 copy 至用戶 personal 層 `~/.claude/skills/aps/`(cross-OS 通用,Claude Code 自己解 `~/`) | 設計或修改 skill 觸發詞、子流程 orchestration、voice 規矩之前 |
