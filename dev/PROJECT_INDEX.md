...
| Package manager | npm (`@adamchanadam/aps` 0.2.4 is published latest and marked as ⚠️ pre-release;0.2.4 adds guided `npx aps init` Traditional Chinese prompts, field-purpose explanations, Google Drive Hub path wording, light branding, Chinese status rows, Chinese starter pack wording, post-install AI first-use guidance, and official-source lookup discipline for setup questions;`.aps/config.json` saved project config and short daily commands remain included) | 2026-05-27 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.4 published pre-release) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents the published npm latest 0.2.4 guided CLI path (`npm install --save-dev @adamchanadam/aps` then `npx aps init`) and marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| `docs/index.html` | non-developer project explainer; user-maintained entry page;current status reflects npm latest 0.2.4 guided CLI setup, short commands, one completed maintainer-run Drive verification, the Agent Handoff Kit `init` confirmation step (`yes`), and the corrected explanation that Hub path means the local Google Drive `AI_Public_Squares` folder path | communicating APS state to humans, e.g. status updates to a counterpart or other stakeholders | local path + GitHub Pages | 2026-05-27 |
| `docs/guides/index.html` | guides hub listing current + planned user-facing walkthroughs;current card reflects npm latest 0.2.4 guided CLI with local config and short-command flow | onboarding a user; adding a new guide | local path | 2026-05-27 |
| `docs/guides/aps-onboarding-walkthrough.html` | end-to-end User A + User B step-by-step onboarding teaching page;now uses the 0.2.4 guided CLI setup path with saved config / short-command flow, explicitly tells users to type `yes` after Agent Handoff Kit `init` and APS `init` show their write plans, and marks day-to-day natural-language flow as pre-release trial use | first-time user reading; pairing with Phase 4 plan during execution | local path | 2026-05-27 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + npm latest 0.2.4 guided CLI path (`npm install`, then `npx aps init`) + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + `yes` confirmation + setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-27 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-27 (`main` includes release commit `9929383d243c49a6ad10d400b4c5985100e5a05a` and the post-publish state-sync commit from this persistence pass;tag `v0.2.4` points to the release commit) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.4 verified 2026-05-27) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-27 |
| GitHub release `v0.2.4` | public pre-release record for APS package version 0.2.4 | before telling users v0.2.4 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.4` | create / update only with explicit release intent | 2026-05-27 (`isPrerelease=true`,`isDraft=false`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-27 (HTTP 200;contains `npm latest 0.2.4 pre-release`;no stale `0.2.3`, `候選`, or placeholder path hits) |
...
| Branch / commit | `main` / release commit `9929383`;release tag `v0.2.4` points to `9929383`;latest commit after this persistence pass records 0.2.4 readbacks | 2026-05-27 |
...
| Uncommitted change summary | None expected after this persistence pass;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
