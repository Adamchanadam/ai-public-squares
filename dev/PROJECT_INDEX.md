...
| Package manager | npm (`@adamchanadam/aps` 0.2.3 is published latest and marked as ⚠️ pre-release;local `package.json` is 0.2.4 candidate for guided `npx aps init` Traditional Chinese prompts, field-purpose explanations, Google Drive Hub path wording, light branding, Chinese status rows, and starter pack wording;0.2.3 adds guided setup, placeholder guard for `init` / `config`, copy-ready `consume` guidance in `inbox`, and guided setup / full-check evidence;`.aps/config.json` saved project config and short daily commands remain included) | 2026-05-27 |
| Test command | `node bin/aps.js --help`, `node bin/aps.js init --dry-run`, `node bin/aps.js config`, `node bin/aps.js bridge-pack --role B`, disposable Hub `init` → saved `.aps/config.json` → short-command `doctor` → `publish` → `inbox` → `consume` → `revise` → `publish` → `withdraw` → `inbox` → `close` → `doctor`, throw-away zero-knowledge tarball install flow, and `node bin/aps.js bogus` smoke test; local v0.3.11 `agent-handoff-kit doctor --root <path>` for governance; manual HTML link audit | 2026-05-27 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.3 published pre-release) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents the published npm latest 0.2.3 guided CLI path (`npm install --save-dev @adamchanadam/aps` then `npx aps init`) and marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI;0.2.3 has guided `npx aps init`, `init --target claude|codex|both` + `--dry-run` skill installer, non-interactive `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, minimal `publish` / `inbox` / `consume` / `close`, `revise`, `withdraw`, read-only `doctor`, saved `.aps/config.json`, `config`, short daily command defaults, and copy-ready `consume` guidance in `inbox`) | when iterating on install / setup orchestration |
...
| APS full audit | `dev/qc/2026-05-27-aps-full-audit-0.2.4.md` (local 0.2.4 guided setup UX candidate full audit, including nested release-check, same-machine regression, external readback boundary, and post-push Pages readback requirement); `dev/qc/2026-05-27-aps-full-audit-0.2.3.md` remains the latest published 0.2.3 full audit; `dev/qc/2026-05-27-aps-full-audit.md` remains the 0.2.1 full audit plus publish readback; `dev/qc/2026-05-26-aps-full-audit.md` remains the 0.2.0 pre-release full audit, real Drive verification, and post-publish readback | when checking whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
...
| `docs/index.html` | non-developer project explainer; user-maintained entry page;current status reflects npm latest 0.2.3 guided CLI setup, short commands, one completed maintainer-run Drive verification, the Agent Handoff Kit `init` confirmation step (`yes`), and the corrected explanation that Hub path means the local Google Drive `AI_Public_Squares` folder path | communicating APS state to humans, e.g. status updates to a counterpart or other stakeholders | local path | 2026-05-27 |
| `docs/guides/index.html` | guides hub listing current + planned user-facing walkthroughs;current card reflects npm latest 0.2.3 guided CLI with local config and short-command flow | onboarding a user; adding a new guide | local path | 2026-05-27 |
| `docs/guides/aps-onboarding-walkthrough.html` | end-to-end User A + User B step-by-step onboarding teaching page;now uses the 0.2.3 guided CLI setup path with saved config / short-command flow, explicitly tells users to type `yes` after Agent Handoff Kit `init` and APS `init` show their write plans, and marks day-to-day natural-language flow as pre-release trial use | first-time user reading; pairing with Phase 4 plan during execution | local path | 2026-05-27 |
...
| `dev/qc/2026-05-27-aps-full-audit-0.2.3.md` | Latest APS full audit report for 0.2.3 guided setup pre-release — records nested quick / release checks, cross-workspace read-only audit, same-machine no-pending regression, CLI `inbox` accept-command UX correction, and npm / git / GitHub release / Pages readback | before asking Adam / Jay to test npm latest | local path | 2026-05-27 |
| `dev/qc/2026-05-27-aps-full-audit-0.2.4.md` | APS full audit report for local 0.2.4 guided setup UX candidate — records nested quick / release checks, corrected public path examples, corrected bundled starter-pack path, same-machine no-pending regression, external readback boundary, and required post-push Pages readback | before committing / publishing the 0.2.4 UX fix | local path | 2026-05-27 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + npm latest 0.2.3 guided CLI path (`npm install`, then `npx aps init`) + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + `yes` confirmation + setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-27 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-27 (`main` pushed to `11e675e2774a29c16eb82ea8466a018f8c300162`;tag `v0.2.3` points to `6e8fb30a4d4d349e20c0832b09d68da1a147b6eb`) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.3 verified 2026-05-27;local `package.json` is 0.2.4 candidate and not yet published) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-27 |
| GitHub release `v0.2.3` | public pre-release record for APS package version 0.2.3 | before telling users v0.2.3 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.3` | create / update only with explicit release intent | 2026-05-27 (`isPrerelease=true`,`isDraft=false`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-27 (HTTP 200;contains `npm latest 0.2.3 pre-release` and no `候選` wording) |
...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init`; placeholder guard for `init` / `config`; guided disposable Hub setup → `doctor` → `publish` → counterpart `inbox` with copy-ready `consume` command → counterpart `consume` → `revise` → counterpart `consume` v2 → `close` → `withdraw` path → no pending inbox; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; `npm test`; `npm pack --dry-run --json`; `git diff --check`; `node bin/aps.js bogus` | every time `bin/aps.js` changes, before committing the change | 2026-05-27 (0.2.3 final full-check regression passed under `dev/qc/evidence/2026-05-27-full-check-023-final2-20260527-123911/`) |
...
| Branch / commit | `main` / latest local commit `86d4990`;release tag `v0.2.3` points to `6e8fb30`;local 0.2.4 guided setup UX candidate remains uncommitted pending Adam release approval | 2026-05-27 |
...
| Uncommitted change summary | Local 0.2.4 candidate touches `package.json`, `bin/aps.js`, README, public docs, APS skill wording, setup dialogue reference, and governance persistence for guided setup UX. Formal 🟡 外發前檢 has passed locally after correcting a public-path example and starter-pack path drift;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
