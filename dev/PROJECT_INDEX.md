...
| Package manager | npm (`@adamchanadam/aps` 0.2.0 published as latest and marked as ⚠️ pre-release; includes `init` as a Claude Code / Codex skill installer, initial Hub skeleton generator, Bridge Pack / starter pack generation, and minimal `publish` / `inbox` / `consume` / `close` CLI round-trip; isolated skill rehearsal, throw-away zero-knowledge flow, and one real Adam ↔ Jay Google Drive round-trip passed) | 2026-05-26 |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI; published 0.2.0 pre-release has `bridge-pack`, `init --target claude|codex|both` + `--dry-run` skill installer, `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, plus minimal `publish` / `inbox` / `consume` / `close` commands) | when iterating on install / setup orchestration |
...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; disposable Hub `publish` → `inbox` → `consume` → reply → `close`; throw-away zero-knowledge tarball install flow; `node bin/aps.js bogus`; post-publish install probe | every time `bin/aps.js` changes, before committing the change | 2026-05-26 (source path, packed-tarball path, isolated skill-level rehearsal, throw-away zero-knowledge flow, formal 🟡 外發前檢, 🔴 全面檢, real Drive round-trip, npm publish, and install probe passed) |
...
| Branch / commit | `main` / 0.2.0 release commit `47bf2d29c8bae7339730d9b655dadbf0b8da64da` pushed;tag `v0.2.0` pushed and points to release commit;post-publish state-sync commit follows on `main` | 2026-05-26 |
...
| Uncommitted change summary | Post-publish state-sync files are being updated;ignored evidence folders remain local under `dev/qc/evidence/`;`adamchanadam-aps-0.2.0.tgz` was generated by `npm pack` in the repo root and should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-26 |
