...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init`; placeholder guard for `init` / `config`; guided disposable Hub setup → `doctor` → `publish` → counterpart `inbox` → counterpart `consume` → `close` → no pending inbox; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; `npm test`; `npm pack --dry-run --json`; `git diff --check`; `node bin/aps.js bogus` | every time `bin/aps.js` changes, before committing the change | 2026-05-27 (local 0.2.3 guided-init regression passed under `dev/qc/evidence/2026-05-27-guided-init-20260527-112028/`; prior 0.2.2 release-check and publish evidence remains historical) |
...
| Branch / commit | `main` / 0.2.2 release commit `b8883cbaab63f3aa42162d66679424e8155b461b`;tag `v0.2.2` points to release commit;local 0.2.3 guided-init candidate uncommitted after passing formal 🟡 外發前檢, pending commit / publish decision | 2026-05-27 |
