...
| Branch / commit | `main` / release commit `6e8fb30`;tag `v0.2.3` points to `6e8fb30`;post-publish state-sync files are uncommitted pending commit / push | 2026-05-27 |
...
| Uncommitted change summary | Post-publish state-sync touches README, public docs, APS skill wording, audit report, and governance persistence so local docs say npm latest 0.2.3 is published pre-release;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
