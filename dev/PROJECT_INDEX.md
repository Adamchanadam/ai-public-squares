...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-28 (`main` includes v0.2.8 release commit `136042c`;tag `v0.2.8` points to that commit) |
...
| GitHub release `v0.2.8` | public GitHub pre-release record for APS source candidate 0.2.8; npm latest remains 0.2.7 until separate publish | before telling users v0.2.8 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.8` | create / update only with explicit release intent | 2026-05-28 (`isPrerelease=true`,`isDraft=false`,`publishedAt=2026-05-28T13:43:05Z`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-28 (HTTP 200;contains `GitHub source 0.2.8`, `npm latest 0.2.7`, `摘要式通知`, and `不會自動觸發對方 AI`) |
...
| Branch / commit | `main` / release commit `136042c`;remote `origin/main` has that commit;release tag `v0.2.8` points to `136042c`;post-release state-sync commit may sit ahead if closeout state is updated after release | 2026-05-28 |
...
| Uncommitted change summary | Release commit `136042c` and tag `v0.2.8` are pushed;GitHub pre-release exists. npm latest remains 0.2.7 because npm publish was not requested. `dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-28 |
