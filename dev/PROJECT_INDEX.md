...
| Branch / commit | `main`;HEAD `b9061c2` (S46 第二段);origin/main `ff044e9` (S44);local 6 ahead, unpushed;tag `v0.2.12`→`c9e8057` latest | 2026-05-29 |
...
| Uncommitted change summary | Working tree clean after `b9061c2`. 6 local commits ahead of origin/main (`ff044e9`), unpushed: `18caf2f` S45 closeout;`387fc5e`+`ac1ab55` 0.2.13 第一段 CLI (no-counterpart model + items contract);`d67dbaa` Agent Handoff Kit 0.3.14 record;`afd7c3a` QC 行為真源對齊機制;`b9061c2` 0.2.13 第二段 skill. `dev/qc/evidence/` is gitignored local evidence (incl. `2026-05-29-codex-0213-stage1-plan/`). Outside repo (not tracked): memory files + `GENERIC_OPERATIONAL_RUNBOOK.md`. Real Hub keeps UAT slugs `aps_uat_0_2_9_jay/` (Jay feedback un-consumed), `aps_uat_claude_20260528/`, + sibling `_aps_uat_adam_recv/` (kept per Adam). Public HTML (README + docs/*.html) still teaches the old five-question model — 第三段 9(d) blocking before any release. | 2026-05-29 |
