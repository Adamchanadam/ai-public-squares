...
| Package manager | npm (`@adamchanadam/aps` 0.2.7 pre-release;0.2.7 includes existing-project `upgrade`, `--body-file` publish / revise, Traditional Chinese help, APS branding, safer receiver notification wording, and Reliable Pair wording hardening) | 2026-05-28 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.7 pre-release) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents the npm latest 0.2.7 guided CLI path (`npm install --save-dev @adamchanadam/aps@latest` then `npx aps init`) and existing-project `npx aps upgrade`;marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| Public product roadmap | `docs/plans/2026-05-28-aps-public-product-multi-agent-roadmap.md` (Reliable Pair + Contacts selector direction; no-cloud-API principle; Contacts only selects target and still writes single-recipient packets; true multi-agent / notify / watch / platform adapters are deferred) | before adding contact selector, group alias, true multi-agent, file-based notification, watch, automation, or platform adapter behavior |
...
| APS full audit | `dev/qc/2026-05-28-aps-full-audit-0.2.7.md` (0.2.7 full audit, including nested release-check, skill refresh, handoff naming split, cloud boundary, same-machine two-workspace regression, HTML sweep, and release gate); older `dev/qc/*full-audit*.md` files remain historical release evidence | when checking whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
