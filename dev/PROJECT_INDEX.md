...
|---|---|---|
| Agent Handoff Kit template version | 0.3.12 | 2026-05-28 |
...
| Package manager | npm (`@adamchanadam/aps` published latest is 0.2.11 pre-release / UAT build — peer-lifecycle root-fix + role→設定起手方向 reframe, published 2026-05-29 S44 on top of the 0.2.9 Project Peers + Sent Status scope + hub-root bracket-validation fix + Codex skill load-fix; prior latest was 0.2.10) | 2026-05-29 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.11;published npm latest is 0.2.11 as of 2026-05-29 S44) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents npm latest 0.2.8 guided CLI path, Agent Handoff Kit preflight before APS init, and local 0.2.9 Project Peers + Sent Status candidate boundary;marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + npm latest 0.2.8 guided CLI path with Agent Handoff Kit preflight (`npx --yes @adamchanadam/agent-handoff-kit@latest init`, then `npm install --save-dev @adamchanadam/aps@latest`, then `npx aps init`) + local 0.2.9 Project Peers + Sent Status candidate boundary + existing-project `npx aps upgrade` + setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-28 |
...
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.11 verified 2026-05-29 S44 publish;bin `aps`;fileCount 14;0.2.11 is the peer-lifecycle root-fix pre-release / UAT build) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-29 |
...
| Worktree or parallel workspace | none from this workspace; siblings `Demo_Agent_{Adam,Jay}_Public_Squares` are independent stores and must be verified in their own roots before relying on their governance health; GitHub Pages enabled at `https://adamchanadam.github.io/ai-public-squares/` | 2026-05-28 |
...
| Uncommitted change summary | local 0.2.9 Project Peers + Sent Status candidate edits are uncommitted;roadmap / governance now also record Project Context Index as a later background-index direction only;starter pack preflight fix is uncommitted;UAT workspaces and G: Hub were modified as authorized test state;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact;S43 added uncommitted Codex skill load-fix edits (`skills/aps/SKILL.md` description 1052→701 + YAML fix, `dev/qc/triggers.md`, `dev/DOC_SYNC_REGISTRY.md`) plus the governance sync (handoff / log / index / archive 009);4 Codex load-fix transcripts saved under `dev/qc/evidence/2026-05-28-codex-skill-loadfix/`;the disposable local UAT scratch dir was deleted while the real Hub test slug `aps_uat_claude_20260528` and overwritten `_hub/starter-pack-{adam,fanny,jay}.md` were left intact;S43 closeout adds the uncommitted 0.2.11 peer-lifecycle root-fix (`bin/aps.js`, `package.json` 0.2.11, `dev/qc/triggers.md`, `dev/DOC_SYNC_REGISTRY.md`, roadmap §4.2, `dev/release-notes/v0.2.9.md` / `v0.2.10.md` / `v0.2.11.md`, full-audit report) and the three rewritten user docs (`docs/index.html`, `docs/guides/index.html`, `docs/guides/aps-onboarding-walkthrough.html`);scratch dir `_aps_uat_adam_recv/` (adam receiver config + codex study transcripts) and real-Hub slug `aps_uat_0_2_9_jay/` remain | 2026-05-28 |
