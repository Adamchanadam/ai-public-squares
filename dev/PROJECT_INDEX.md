...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + current 0.2.2 CLI-first path (`npm install`, `aps init --dry-run`, then full `aps init --hub-root ...`) + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + `yes` confirmation + deep-dive references | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-27 |
