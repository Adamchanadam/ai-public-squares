...
| Package manager | npm (`@adamchanadam/aps` 0.2.8 local GitHub pre-release candidate; npm latest remains 0.2.7 until separate npm publish; 0.2.8 candidate adds role-B identity default fix plus post-UAT Reliable Pair notification / receiver-alignment hardening) | 2026-05-28 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current published npm path remains `npm install --save-dev @adamchanadam/aps@latest` then guided `npx aps init` for npm latest 0.2.7;local 0.2.8 GitHub candidate adds role-B guided setup default `agent_b / agent_a`, product alias discovery, agent identity validation, and summary-first notification text for the next patch release | when iterating on install / setup flow, CLI packet flow, or before npm publish |
| `skills/` | APS skill source — `skills/aps/SKILL.md` 是 target orchestration spec;`skills/aps/references/setup-dialogue.md` 是隨 npm package 出貨的 setup wording bank;`init` copies this folder into supported AI tool directories and `upgrade` refreshes existing installs. Setup routes through guided `npx aps init`;existing-project updates route through `npm install --save-dev @adamchanadam/aps@latest` then `npx aps upgrade`;post-install first-use orchestration makes AI read `.aps/config.json`, run `doctor` / `inbox`, offer test packet / starter pack / daily-use next steps, treat `Agent Public Squares` as a natural-language alias of `AI Public Squares`, keep APS CLI version separate from Agent Handoff Kit version, avoid Agent Handoff Kit banner inside APS skill output, require full content + topic + Hub-write confirmation before publish, prefer `--body-file` for long generated packet bodies, generate summary-first human notifications, and make `check Hub` produce a receiver-side report with completeness + local alignment checks before work starts. Daily publish / inbox / consume / close remain aligned to the short-command CLI;recovery wording uses `revise`, `withdraw`, and `doctor`. Natural-language daily operation and recovery are still not production-complete | 設計或修改 skill 觸發詞、子流程 orchestration、voice 規矩之前 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.8 local pre-release candidate; npm latest still 0.2.7 until publish) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents that GitHub source is 0.2.8 candidate while npm latest remains 0.2.7;keeps guided CLI path (`npm install --save-dev @adamchanadam/aps@latest` then `npx aps init`) and existing-project `npx aps upgrade`;marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI;local 0.2.8 candidate has guided Traditional Chinese `npx aps init`, safer `publish --body-file` / `revise --body-file` path for long generated packet bodies while preserving `--body`, plus `npx aps upgrade` for existing projects to refresh skills / bridge / Handoff Kit registration without overwriting Hub packets;CLI includes `init --target claude|codex|both` + `--dry-run` skill installer, non-interactive `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, minimal `publish` / `inbox` / `consume` / `close`, `revise`, `withdraw`, read-only `doctor`, saved `.aps/config.json`, `config`, short daily command defaults, copy-ready summary notification, Handoff Kit APS registration, APS brand card, emoji status output, Traditional Chinese help, and role-B identity default fix) | when iterating on install / setup orchestration |
...
| Public product roadmap | `docs/plans/2026-05-28-aps-public-product-multi-agent-roadmap.md` (Reliable Pair first; no-cloud-API principle; Reliable Pair uses `.aps/config.json` `otherAgentId` rather than Contacts selector; summary-style human notifications and receiver-side local alignment report are part of Reliable Pair; Contacts selector, true multi-agent, file-based notify, watch, Telegram bot auto-send, and platform adapters are deferred or opt-in future work) | before adding contact selector, group alias, true multi-agent, file-based notification, watch, automation, or platform adapter behavior |
...
| APS full audit | `dev/qc/2026-05-28-aps-full-audit-0.2.7-post-uat.md` (latest post-UAT full-check: local runtime regression passed after role-B identity-default fix, but release pass is blocked because GitHub Pages is stale and npm latest is already 0.2.7); `dev/qc/2026-05-28-aps-full-audit-0.2.7.md` remains the original 0.2.7 release-prep audit | when checking whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
...
| `docs/plans/2026-05-28-aps-public-product-multi-agent-roadmap.md` | APS public product roadmap — defines the post-0.2.x direction as Reliable Pair first, then Contacts selector; Reliable Pair now includes summary-style human notification and receiver-side `check Hub` report / local alignment check, using `.aps/config.json` `otherAgentId` as the single counterpart; Contacts selector later selects target but keeps single-recipient packets; true multi-agent collaboration, file-based notification, `watch`, Telegram bot auto-send, platform adapters, Dropbox / OneDrive support, and the no-cloud-API core principle are scoped and gated | before adding or publicly promising Contacts selector, true multi-agent, group alias, notification, watch, automation, Dropbox / OneDrive support, or AI-platform scheduled task support | local path + official platform docs linked inside | 2026-05-28 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + npm latest 0.2.7 guided CLI path (`npm install --save-dev @adamchanadam/aps@latest`, then `npx aps init`) + existing-project `npx aps upgrade` + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-28 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-28 (`main` includes release commit `e723dfc`;tag `v0.2.6` points to that commit) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.7 verified 2026-05-28;bin `aps`;fileCount 14) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-28 |
| GitHub release `v0.2.6` | public pre-release record for APS package version 0.2.6 | before telling users v0.2.6 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.6` | create / update only with explicit release intent | 2026-05-28 (`isPrerelease=true`,`isDraft=false`,`publishedAt=2026-05-28T06:27:25Z`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-28 (HTTP 200 via Node HTTPS readback;contains `0.2.6` and pre-release wording;no `0.2.5 pre-release` latest wording) |
...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init`; placeholder guard for `init` / `config`; guided disposable Hub setup → `upgrade --dry-run` → `upgrade` → `doctor` → `publish --body-file` → counterpart `inbox` with copy-ready `consume` command → counterpart `consume` → `revise --body-file` → counterpart `consume` v2 → `close` → `withdraw` path → no pending inbox; `publish --agent-id` / `--other-agent-id` override alias; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; `npm test`; `npm pack --dry-run --json`; `git diff --check`; `node bin/aps.js bogus`; published install probe / UAT install probe | every time `bin/aps.js` changes, before committing the change | 2026-05-28 (0.2.7 release-check passed: package syntax / pack / CLI smoke / help / HTML render / npm latest pre-readback) |
| Claude Code headless external review | `cmd /c claude -p "<read-only review prompt>"` from this repo root. Keep the prompt bounded, name the files to read, and explicitly say `只讀，不改檔`. If it times out, retry with a shorter prompt and fewer files before treating it as blocked. If a technical blocker remains, verify current Claude Code official docs or web guidance before changing this command pattern. | when Adam asks for an independent Claude review, or before continuing after a broad UX / governance design change where a second AI review is useful | 2026-05-28 (`cmd /c claude -p ...` succeeded after an earlier overlong prompt timed out; returned read-only Reliable Pair UX review) |
...
| Branch / commit | `main` / latest local release-prep commit `30c3a81`;latest pushed commit remains `d347490`;release tag `v0.2.6` still points to `e723dfc`;no v0.2.7 tag or GitHub release yet | 2026-05-28 |
...
| Uncommitted change summary | Post-0.2.7 local hardening is uncommitted: Reliable Pair UX docs / skill / CLI notification changes, role-B interactive identity-default fix, full-check report, and governance state updates. GitHub Pages is stale until push; npm latest is already 0.2.7, so any future npm publish needs a later version. Commit / push / tag / GitHub release / npm publish only if Adam explicitly asks. `dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-28 |
