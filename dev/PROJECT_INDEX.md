...
| Package manager | npm (`@adamchanadam/aps` 0.2.1 published as latest and marked as ⚠️ pre-release;local package.json is 0.2.2 candidate with `.aps/config.json` saved local config and short daily commands; isolated skill rehearsal, throw-away zero-knowledge flow, one real Adam ↔ Jay Google Drive round-trip, and local 0.2.2 short-command regression passed) | 2026-05-27 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current verified published path is `npm install --save-dev @adamchanadam/aps` then `npx aps ...`;published 0.2.1 pre-release includes `bridge-pack`, `init`, minimal packet CLI, `revise`, `withdraw`, and read-only `doctor`;local 0.2.2 candidate additionally writes `.aps/config.json` during setup and lets daily commands omit Hub / project / agent long flags | when iterating on install / setup flow, CLI packet flow, or before npm publish |
| `skills/` | APS skill source — `skills/aps/SKILL.md` 是 target orchestration spec;`skills/aps/references/setup-dialogue.md` 是隨 npm package 出貨的 setup wording bank;`init` copies this folder into supported AI tool skill directories. Setup and daily publish / inbox / consume / close are aligned to the local 0.2.2 short-command CLI while preserving published 0.2.1 boundary;recovery wording uses `revise`, `withdraw`, and `doctor`. Natural-language daily operation and recovery are still not production-complete | 設計或修改 skill 觸發詞、子流程 orchestration、voice 規矩之前 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;local version 0.2.2 candidate, npm latest remains 0.2.1 until publish) | when adjusting package metadata, version, or publish channel |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI; published 0.2.1 pre-release has `bridge-pack`, `init --target claude|codex|both` + `--dry-run` skill installer, `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, plus minimal `publish` / `inbox` / `consume` / `close`, `revise`, `withdraw`, and read-only `doctor`;local 0.2.2 candidate adds saved `.aps/config.json`, `config`, and short daily command defaults) | when iterating on install / setup orchestration |
...
| `docs/guides/index.html` | guides hub listing current + planned user-facing walkthroughs;current card reflects npm latest 0.2.1 pre-release plus local 0.2.2 candidate short-command boundary | onboarding a user; adding a new guide | local path | 2026-05-27 |
...
| `docs/plans/2026-05-23-aps-skill-dialogue-script.md` | APS skill dialogue companion — `skills/aps/SKILL.md` 之 wording bank reference;Setup / 發佈 / 收件 / 補救 4 子流程 sample dialogue now aligned to the local 0.2.2 short-command CLI packet folder / outbox / ack model while preserving npm 0.2.1 published boundary(中文版 land,英文版 defer);概念 inject 順序圖 + 語氣指南(做與不做) | 設計或修改 skill 觸發後對用戶之 voice / 對話順序 / 概念解釋之前 | local path | 2026-05-27 |
...
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-27 (HTTP 200;contains `0.2.1` and pre-release / 前期測試 wording) |
...
| Branch / commit | `main` / current working tree has local 0.2.2 candidate changes not yet committed;0.2.1 release commit `d9dd1aa1acf2b2f5c863056382543922c5c0aeb4`;tag `v0.2.1` points to release commit | 2026-05-27 |
