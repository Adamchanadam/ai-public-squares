...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/agent-public-squares.git` (renamed from `ai-public-squares` 2026-05-29; HTTPS;auth via Windows Credential Manager,auto-pass;GitHub auto-redirects old web+git URLs but NOT Pages/Actions — do not reuse the old name) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-29 (origin/main `ff044e9` S44;local HEAD `b9061c2` S46, 6 ahead unpushed;tag `v0.2.12`→`c9e8057` latest, `v0.2.8`→`136042c` retained) |
...
| Branch / commit | `main`;HEAD `10bd98b` (S47 B4/B5);origin/main `ff044e9` (S44);local 9 ahead, unpushed;tag `v0.2.12`→`c9e8057` latest;`package.json` still 0.2.12 (bump at 第五段) | 2026-05-29 |
...
| Uncommitted change summary | Working tree clean after `10bd98b`. 9 local commits ahead of origin/main (`ff044e9`), unpushed: `18caf2f` S45;`387fc5e`+`ac1ab55` 0.2.13 第一段;`d67dbaa` Kit 0.3.14;`afd7c3a` QC 9(d);`b9061c2` 0.2.13 第二段;`3ff71b3` S46 closeout;`3d1dcd1` 0.2.13 第三段+ (public surfaces + 命名統一 Agent_Public_Squares + starter pack + 加入教學頁);`10bd98b` 0.2.13 B4/B5. 0.2.13 第一–四段 done & committed + 第四段 UAT GREEN;只剩第五段 gated release;`package.json` 仍 0.2.12. `dev/qc/evidence/` is local evidence (incl. `2026-05-29-codex-starterpack-review/`, not committed). Outside repo (not tracked): memory files + `GENERIC_OPERATIONAL_RUNBOOK.md`. The S47 UAT sandbox is in OS temp (`%LOCALAPPDATA%\Temp\claude\tmp.L2ZSZPNfCZ`), outside the repo. Real Hub keeps UAT slugs (Jay feedback un-consumed; Adam confirmed disposable UAT). Public surfaces now aligned to three-question / items — 9(d) PASSED (prior public-HTML blocker cleared). | 2026-05-29 |
