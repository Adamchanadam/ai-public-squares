...
| Package manager | npm (`@adamchanadam/aps` 0.2.0 published as latest and marked as ⚠️ pre-release;local package.json is now 0.2.1 candidate with `revise`, `withdraw`, and read-only `doctor`; isolated skill rehearsal, throw-away zero-knowledge flow, and one real Adam ↔ Jay Google Drive round-trip passed for the 0.2.0 release scope) | 2026-05-26 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current verified published path is `npm install --save-dev @adamchanadam/aps` then `npx aps ...`;published 0.2.0 pre-release includes `bridge-pack`, `init` skill installation for Claude Code / Codex, initial Hub skeleton / Bridge Pack / starter pack generation, and minimal CLI `publish` / `inbox` / `consume` / `close` round-trip. Local 0.2.1 candidate additionally has `revise`, `withdraw`, and read-only `doctor` | when iterating on install / setup flow, CLI packet flow, or before npm publish |
| `skills/` | APS skill source — `skills/aps/SKILL.md` 是 target orchestration spec;`skills/aps/references/setup-dialogue.md` 是隨 npm package 出貨的 setup wording bank;`init` copies this folder into supported AI tool skill directories. Setup and daily publish / inbox / consume / close are aligned to the published 0.2.0 pre-release CLI;local 0.2.1 candidate also aligns recovery wording to `revise`, `withdraw`, and `doctor`. Natural-language daily operation and recovery are still not production-complete | 設計或修改 skill 觸發詞、子流程 orchestration、voice 規矩之前 |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI; published 0.2.0 pre-release has `bridge-pack`, `init --target claude|codex|both` + `--dry-run` skill installer, `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, plus minimal `publish` / `inbox` / `consume` / `close` commands;local 0.2.1 candidate adds `revise`, `withdraw`, and read-only `doctor`) | when iterating on install / setup orchestration |
...
| `docs/index.html` | non-developer project explainer; user-maintained entry page;current status reflects 0.2.0 pre-release CLI-first setup and one completed maintainer-run Drive verification | communicating APS state to humans, e.g. status updates to a counterpart or other stakeholders | local path | 2026-05-26 |
...
| `docs/guides/aps-onboarding-walkthrough.html` | end-to-end User A + User B step-by-step onboarding teaching page;now uses the 0.2.0 CLI-first setup path and marks day-to-day flow as pre-release trial use | first-time user reading; pairing with Phase 4 plan during execution | local path | 2026-05-26 |
...
| `dev/qc/2026-05-27-aps-full-audit.md` | Latest APS full audit report for 0.2.1 local candidate — records nested release-check, cross-workspace read-only audit, local revise / withdraw / doctor regression, HTML preview, registry / Pages / remote readback, and remaining pre-release boundaries | before pushing, tagging, creating GitHub release, or npm publishing 0.2.1 | local path | 2026-05-27 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + current 0.2.0 CLI-first path + deep-dive references | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-26 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-26 (`main` pushed after 0.2.0 release;tag `v0.2.0` points to `47bf2d29c8bae7339730d9b655dadbf0b8da64da`) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.0 verified 2026-05-26) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-26 |
| GitHub release `v0.2.0` | public pre-release record for APS package version 0.2.0 | before telling users v0.2.0 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.0` | create / update only with explicit release intent | 2026-05-26 (`isPrerelease=true`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-26 (HTTP 200;contains `0.2.0` and pre-release wording) |
...
| Branch / commit | `main` / current `HEAD` is local 0.2.1 candidate commit `Prepare APS 0.2.1 local candidate` and is not pushed;0.2.0 release commit `47bf2d29c8bae7339730d9b655dadbf0b8da64da`;tag `v0.2.0` points to release commit | 2026-05-27 |
...
| Uncommitted change summary | None at the commit point;public new-user walkthrough correction and local 0.2.1 CLI hardening are committed locally at current `HEAD` but not pushed;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
