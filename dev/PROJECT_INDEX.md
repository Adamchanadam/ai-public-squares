...
| Package manager | npm (`@adamchanadam/aps` 0.2.2 published as latest and marked as ⚠️ pre-release;`.aps/config.json` saved project config and short daily commands included; isolated skill rehearsal, throw-away zero-knowledge flow, one real Adam ↔ Jay Google Drive round-trip, 0.2.2 short-command regression, and published install probe passed;local main has an unpublished placeholder-guard fix for the next CLI patch) | 2026-05-27 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current verified path is `npm install --save-dev @adamchanadam/aps` then `npx aps ...`;0.2.2 pre-release includes `bridge-pack`, `init`, minimal packet CLI, `revise`, `withdraw`, read-only `doctor`, saved `.aps/config.json`, `config`, and short daily commands | when iterating on install / setup flow, CLI packet flow, or before npm publish |
| `skills/` | APS skill source — `skills/aps/SKILL.md` 是 target orchestration spec;`skills/aps/references/setup-dialogue.md` 是隨 npm package 出貨的 setup wording bank;`init` copies this folder into supported AI tool skill directories. Setup and daily publish / inbox / consume / close are aligned to the 0.2.2 short-command CLI;recovery wording uses `revise`, `withdraw`, and `doctor`. Natural-language daily operation and recovery are still not production-complete | 設計或修改 skill 觸發詞、子流程 orchestration、voice 規矩之前 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.2 release-prep) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents the current 0.2.2 CLI-first path (`npm install --save-dev @adamchanadam/aps` then `npx aps init --dry-run` / `npx aps init --hub-root ...`) and marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI;0.2.2 pre-release has `bridge-pack`, `init --target claude|codex|both` + `--dry-run` skill installer, `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, minimal `publish` / `inbox` / `consume` / `close`, `revise`, `withdraw`, read-only `doctor`, saved `.aps/config.json`, `config`, and short daily command defaults) | when iterating on install / setup orchestration |
...
| `docs/index.html` | non-developer project explainer; user-maintained entry page;current status reflects 0.2.2 pre-release CLI-first setup, short commands, one completed maintainer-run Drive verification, and the Agent Handoff Kit `init` confirmation step (`yes`) | communicating APS state to humans, e.g. status updates to a counterpart or other stakeholders | local path | 2026-05-27 |
| `docs/guides/index.html` | guides hub listing current + planned user-facing walkthroughs;current card reflects npm latest 0.2.2 pre-release with local config and short-command flow | onboarding a user; adding a new guide | local path | 2026-05-27 |
| `docs/guides/aps-onboarding-walkthrough.html` | end-to-end User A + User B step-by-step onboarding teaching page;now uses the 0.2.2 CLI-first setup path with saved config / short-command flow, explicitly tells users to type `yes` after Agent Handoff Kit `init` shows its write plan, and marks day-to-day natural-language flow as pre-release trial use | first-time user reading; pairing with Phase 4 plan during execution | local path | 2026-05-27 |
...
| `dev/qc/2026-05-27-aps-full-audit.md` | Latest APS full audit report for 0.2.1 pre-release — records nested release-check, cross-workspace read-only audit, local revise / withdraw / doctor regression, HTML preview, push / tag / GitHub pre-release / npm publish readback, and remaining pre-release boundaries. Local 0.2.2 short-command evidence lives under ignored `dev/qc/evidence/2026-05-27-config-shortcmd-*` until a new release audit is created | before claiming 0.2.1 release health, 0.2.2 release readiness, or production readiness | local path | 2026-05-27 |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + current 0.2.2 CLI-first path (`npm install`, `aps init --dry-run`, then full `aps init --hub-root ...`) + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + `yes` confirmation + four setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-27 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-27 (`main` pushed for 0.2.2;tag `v0.2.2` points to `b8883cbaab63f3aa42162d66679424e8155b461b`) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.2 verified 2026-05-27) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-27 |
| GitHub release `v0.2.2` | public pre-release record for APS package version 0.2.2 | before telling users v0.2.2 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.2` | create / update only with explicit release intent | 2026-05-27 (`isPrerelease=true`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-27 (HTTP 200;contains `0.2.2` and pre-release / 前期測試 wording) |
...
| npm CLI smoke test | `node bin/aps.js --help`; `node bin/aps.js init --dry-run`; `node bin/aps.js bridge-pack --role B`; disposable Hub `publish` → `inbox` → `revise` → `inbox` → `withdraw` → `inbox` → `publish` → `consume` → `close` → `doctor`; throw-away zero-knowledge tarball install flow; `node bin/aps.js bogus`; post-publish install probe; 0.2.2 saved-config short-command flow and `config` command tests | every time `bin/aps.js` changes, before committing the change | 2026-05-27 (source path, packed-tarball path, isolated skill-level rehearsal, throw-away zero-knowledge flow, formal 🟡 外發前檢, 🔴 全面檢, real Drive round-trip, npm publish, install probe, 0.2.1 `revise` / `withdraw` / `doctor` smoke pass, formal 0.2.1 外發前檢, and formal 0.2.2 外發前檢 passed) |
...
| Branch / commit | `main` / 0.2.2 release commit `b8883cbaab63f3aa42162d66679424e8155b461b`;tag `v0.2.2` points to release commit | 2026-05-27 |
...
| Uncommitted change summary | Expected clean after 0.2.2 post-publish state-sync commit;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-27 |
