...
| Package manager | npm (`@adamchanadam/aps` 0.2.6 is published latest and marked as ⚠️ pre-release;0.2.6 includes clearer `doctor` next-step guidance, fuller CLI fallback command list, Traditional Chinese success messages, `@latest` install wording for upgrade clarity, and the side-border-free connected-cat setup banner) | 2026-05-28 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current published path is `npm install --save-dev @adamchanadam/aps@latest` then guided `npx aps init`;0.2.6 includes versioned install / doctor output, `agent_a` default, Traditional Chinese `doctor`, rerun-safe update of generated local config surfaces, Handoff Kit startup discovery registration via APS managed blocks in `dev/RULE_PACKS.md` + `dev/PROJECT_INDEX.md`, clearer `doctor` next-step guidance, fuller CLI fallback listing, remaining Chinese success messages, and a side-border-free connected-cat setup banner | when iterating on install / setup flow, CLI packet flow, or before npm publish |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;published latest is 0.2.6 and still marked as pre-release) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents the published npm latest 0.2.6 guided CLI path (`npm install --save-dev @adamchanadam/aps@latest` then `npx aps init`) and marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| Entry | Path | Notes |
|---|---|---|
| App entry | `bin/aps.js` (npm package CLI;0.2.6 has guided Traditional Chinese `npx aps init`, `init --target claude|codex|both` + `--dry-run` skill installer, non-interactive `--hub-root --project --agent-id --other-agent-id --role` Hub skeleton setup, minimal `publish` / `inbox` / `consume` / `close`, `revise`, `withdraw`, read-only `doctor`, saved `.aps/config.json`, `config`, short daily command defaults, copy-ready notification, Handoff Kit APS registration, copy-ready `consume` guidance in `inbox`, clearer post-doctor guidance, and side-border-free connected-cat setup banner) | when iterating on install / setup orchestration |
...
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.6 verified 2026-05-28) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-28 |
