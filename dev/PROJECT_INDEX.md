...
| Package manager | npm (`@adamchanadam/aps` 0.2.8 pre-release; 0.2.8 adds role-B identity default fix plus post-UAT Reliable Pair notification / receiver-alignment hardening) | 2026-05-28 |
...
| `bin/` | npm package executable entry; `bin/aps.js` exposes `aps`; current published npm path is `npm install --save-dev @adamchanadam/aps@latest` then guided `npx aps init`;0.2.8 adds role-B guided setup default `agent_b / agent_a`, product alias discovery, agent identity validation, and summary-first notification text | when iterating on install / setup flow, CLI packet flow, or before npm publish |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;version 0.2.8 pre-release) | when adjusting package metadata, version, or publish channel |
| `README.md` | GitHub first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary;documents npm latest 0.2.8 guided CLI path (`npm install --save-dev @adamchanadam/aps@latest` then `npx aps init`) and existing-project `npx aps upgrade`;marks natural-language daily use as not production-complete | when shipping a Layer A change (entry framing / install command / status table) |
...
| APS full audit | `dev/qc/2026-05-28-aps-full-audit-0.2.7-post-uat.md` records the post-UAT regression that led to v0.2.8: local runtime regression passed after role-B identity-default fix, and the then-active release blockers were stale GitHub Pages plus immutable npm 0.2.7; those external blockers were cleared by the later v0.2.8 GitHub pre-release and npm publish. `dev/qc/2026-05-28-aps-full-audit-0.2.7.md` remains the original 0.2.7 release-prep audit | when checking whether APS can be treated as fully verified before cross-workspace work, protocol promotion, public release, or new-agent introduction |
...
| `README.md` | GitHub repo first-impression entry — zero-knowledge friendly 痛點 hook + pre-release boundary + npm latest 0.2.8 guided CLI path (`npm install --save-dev @adamchanadam/aps@latest`, then `npx aps init`) + existing-project `npx aps upgrade` + upstream-aligned Agent Handoff Kit command (`npx --yes @adamchanadam/agent-handoff-kit@latest init`) + setup values explained so readers do not copy placeholder paths | first stop for any visitor to the GitHub repo; Layer A primary deliverable | local path + https://github.com/Adamchanadam/ai-public-squares#readme | 2026-05-28 |
...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/ai-public-squares.git` (HTTPS;auth via Windows Credential Manager,auto-pass) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-28 (`main` includes v0.2.8 release / npm-publish state;tag `v0.2.8` points to release commit `136042c`) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.8 verified 2026-05-28;bin `aps`;fileCount 14) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-28 |
| GitHub release `v0.2.8` | public GitHub pre-release record for APS 0.2.8 | before telling users v0.2.8 is released through GitHub | `https://github.com/Adamchanadam/ai-public-squares/releases/tag/v0.2.8` | create / update only with explicit release intent | 2026-05-28 (`isPrerelease=true`,`isDraft=false`,`publishedAt=2026-05-28T13:43:05Z`; body updated after npm publish to state npm latest `0.2.8`) |
| GitHub Pages | hosted public HTML entry surface | after pushing public docs changes | `https://adamchanadam.github.io/ai-public-squares/docs/index.html` | writes happen via `main` branch push only | 2026-05-28 (HTTP 200;contains `npm latest 0.2.8`, `摘要式通知`, and `不會自動觸發對方 AI`) |
...
| Branch / commit | `main`;release tag `v0.2.8` points to release commit `136042c`;later `main` commits record GitHub release and npm publish state sync | 2026-05-28 |
...
| Uncommitted change summary | v0.2.8 npm publish state-sync edits should be committed and pushed;`dev/qc/evidence/` is ignored local evidence;`adamchanadam-aps-*.tgz` generated by `npm pack` should not be committed unless Adam explicitly wants to keep a tarball artifact | 2026-05-28 |
