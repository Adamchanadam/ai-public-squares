...
| Package manager | npm (`@adamchanadam/aps` 0.2.4 is published latest and marked as ⚠️ pre-release;local `package.json` is 0.2.5 candidate after UAT fixes:versioned install banner, connected-cat ASCII, `agent_a` default, Traditional Chinese `doctor`, confirmed update of `.aps/config.json` / Bridge Pack / starter pack when `init` is rerun, and Handoff Kit discovery registration via APS managed blocks in `dev/RULE_PACKS.md` + `dev/PROJECT_INDEX.md`;`.aps/config.json` saved project config and short daily commands remain included) | 2026-05-27 |
...
| `package.json` | npm package manifest (`@adamchanadam/aps`, Apache-2.0, Node ≥18, bin entry `aps`;published latest is 0.2.4;local version is 0.2.5 candidate) | when adjusting package metadata, version, or publish channel |
...
| `dev/qc/2026-05-28-aps-full-audit-0.2.5.md` | Latest local APS full audit report for 0.2.5 candidate — records nested quick / release checks, Class-C read-only audit, `publish --agent-id` / `--other-agent-id` alias root-fix, same-machine two-workspace no-pending regression, and the commit-before-formal-release gate | before committing, publishing, or asking Adam / Jay to test 0.2.5 from npm latest | local path | 2026-05-28 |
...
| Branch / commit | `main` / latest pushed commit `52e2c3a`;release tag `v0.2.4` points to `9929383`;local 0.2.5 UAT fixes are uncommitted | 2026-05-27 |
