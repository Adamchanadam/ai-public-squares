...
| GitHub remote `origin` | public repo hosting full git history, release tags, GitHub Pages, and Apache-2.0 license | every push / pull; before any GitHub release or Pages verification | `https://github.com/Adamchanadam/agent-public-squares.git` (renamed from `ai-public-squares` 2026-05-29; HTTPS;auth via Windows Credential Manager,auto-pass;GitHub auto-redirects old web+git URLs but NOT Pages/Actions — do not reuse the old name) | `git push origin main` and version tag pushes only with explicit release intent;no force-push without Adam 明示 | 2026-05-29 (`main` == origin == `ff044e9` S44 closeout;tag `v0.2.12`→`c9e8057` latest, `v0.2.8`→`136042c` retained;S45 working tree dirty + unpushed) |
| npm registry — `@adamchanadam/aps` scope | distribution channel for the APS CLI package | before documenting npm commands or changing package version | npm registry `https://www.npmjs.com/package/@adamchanadam/aps` (`latest` = 0.2.12 verified 2026-05-29 S44 publish;bin `aps`;fileCount 14;0.2.12 is the naming & framing pre-release;S45 made no release) | `npm publish --access public` from this workspace root; verify with `npm view @adamchanadam/aps version dist-tags.latest bin dist.fileCount --json` | 2026-05-29 |
