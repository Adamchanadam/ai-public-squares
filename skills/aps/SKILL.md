...
> **Status — target orchestration spec, not current runtime guarantee (2026-05-25):** Section 1-8 中文版第一版已存在,對話具體 wording 詳見 `docs/plans/2026-05-23-aps-skill-dialogue-script.md`;英文版 deferred。npm package `@adamchanadam/aps` 尚未 publish,`init` 仍為佔位符,本 SKILL.md 不會自動 install 至用戶 `~/.claude/skills/aps/`。以下流程描述目標行為與實作規格,不得對使用者承諾已可由公開 npm 命令直接執行。

## 1. 此 skill 的職責

帶用戶行完跨機合作的整個生命周期:初次設置(與對方各自接駁一個共用的雲端資料夾)、日常一來一往(交檔、收檔)、出錯時的補救。用戶多數對協定內部結構毫無概念,亦不應該需要知道。Skill 的目標是將整套協定包成「用戶答幾條問題、其餘自動」的形態。**目前此檔仍是目標規格,不是已經通過 runtime testing 的能力清單;任何檔案操作、starter pack 生成、日常發佈或補救動作,實作前須以當前 CLI 與 bundled files 重新驗證。**
...
5. **執行檔案操作**(§1.5)— 進度逐步回報,每步用 ⏳ 標記:
   ...
   - 複製 PROTOCOL.md v1.0 到 `<slug>/_hub/PROTOCOL.md`。目前 repo 未提供 bundled PROTOCOL source file;實作前須先新增該 source file 或改由現有 Hub / template source 讀取,不得假裝此資源已存在。
